import numpy as np
import click
import pandas as pd
import matplotlib.pyplot as plt
plt.style.use('ggplot')


def expected_events(index, N, bin_edges, log=True):
    events = []
    for e_low, e_high in zip(bin_edges[0:], bin_edges[1:]):
        if log:
            e_low, e_high = 10**e_low, 10**e_high

        e = (1 / (index + 1)) * e_high**(index + 1) - \
            (1 / (index + 1)) * e_low**(index + 1)
        # e = e_high**(index) - e_low**index
        events.append(e)

    events = np.array(events)
    total = np.sum(events)

    return N / total * events


def plot(bin_edges, energy, expectation):
    fig, (ax, ax2) = plt.subplots(2, 1)
    bin_center = 0.5 * (bin_edges[:-1] + bin_edges[1:])
    bin_width = np.diff(bin_edges)

    ax.set_yscale('log')
    ax.errorbar(bin_center, expectation, xerr=bin_width *
                0.5, marker='.', linestyle='', capsize=0)
    H, _, _ = ax.hist(energy, bins=len(bin_center))

    ax2.errorbar(bin_center, H / expectation, xerr=bin_width *
                 0.5, marker='.', linestyle='', capsize=0, color='0.25')

    ax2.set_ylabel('Efficiency')
    ax2.set_xlabel(r'$\log_{10}(E /  \mathrm{GeV})$')

    return fig, (ax, ax2)


@click.command()
@click.argument('predicted_events', type=click.Path(exists=True, dir_okay=False,))
@click.argument('mc_production_information', type=click.Path(exists=True))
@click.argument('outputfile', type=click.Path(exists=False, dir_okay=False,))
@click.option('-n', '--n_energy', type=click.INT, default=11, help='energy bins')
@click.option('-t', '--prediction_threshold', default=0.9, help='mean "gammaness"')
def main(
        predicted_events,
        mc_production_information,
        outputfile,
        n_energy,
        prediction_threshold,
):
    '''
    Plot the event distributions from the predicted gammas given in the
    PREDICTED_EVENTS input file.
    '''
    predictions = pd.read_csv(predicted_events)
    s = predictions['prediction:signal:mean'] > 0.6
    predictions = predictions[s]
    mc = pd.read_csv(mc_production_information)

    simulated_showers = mc.query(
        'file_names in @predictions.source_file.unique()'
        ) \
        .simulated_showers \
        .sum()

    # energy is stored in TeV apparently. lets make it GeV and log it
    energy = (predictions['mc:energy'] * 1000).apply(np.log10)

    _, edges = np.histogram(energy, bins=n_energy)

    expectation = expected_events(-2, simulated_showers, edges)

    fig, _ = plot(edges, energy, expectation)
    fig.savefig(outputfile)


if __name__ == '__main__':
    main()
