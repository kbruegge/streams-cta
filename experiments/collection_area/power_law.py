...
class MCSpectrum(Spectrum):
    ...
    def simulated_mc_events_per_bin(self, bins=10, log=True):
        '''
        Get the number of events which are expected to arrive from this spectral source.
        For each of the requested bins.
        Parameters
        ...
        log: boolean
            Whether to use logarithmically spaced bins or not.
            So when log == True this will use np.logspace to create the bins.
        '''
        a = self.e_min.to('TeV').value
        b = self.e_max.to('TeV').value
        if log:
            edges = np.logspace(np.log10(a), np.log10(b), num=bins, base=10.0) * u.TeV
        else:
            edges = np.linspace(a, b, num=bins)

        events = []
        for e_low, e_high in zip(edges[0:], edges[1:]):
            N = self._integral(e_low, e_high) * u.m**2 * u.s
            events.append(N.si.value)

        N = self._integral(self.e_min, self.e_max) * u.m**2 * u.s
        N = N.si.value

        events = np.array(events)
        events = (self.total_showers_simulated / N) * events
