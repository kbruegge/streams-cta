...
class Spectrum():
    ...
    def draw_energy_distirbution(self, e_min, e_max, shape):
        ...
        g = self.index
        e = (np.random.uniform(a, b, shape) * (g + 1))**(1.0/(g+1))
        return e*u.TeV
    ...
    @u.quantity_input(event_energies=u.TeV, e_max_simulated=u.TeV, e_min_simulated=u.TeV, area=u.m**2)
    def weight(self, event_energies, e_min_simulated, e_max_simulated, area, simulated_showers ,simulated_index=-2.0):
        event_energies = event_energies.to('TeV')
        e_0 = self._correction_factor(e_min_simulated, e_max_simulated, simulated_index)
        rate = self.expected_events(e_min_simulated, e_max_simulated, area, t_obs=1*u.s) / u.s
        w = (event_energies / e_0)**(self.index - simulated_index) * rate / simulated_showers;
        w = w.to(1/u.s)
        return w
