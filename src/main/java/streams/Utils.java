...
public class Utils {
    ...
    /**
     * Transform 1D array into 2D array (pixel array with an array of slices for each pixel)
     *
     * @param data     1D array
     * @param numPixel number of pixels in the telescope
     *
     * @return 2D array
     */
    ...
    /**
     * This takes a data array (of length pixels * roi) and returns an array(length = roi) in which
     * each entry is the average over all the values of the other pixels in that slice.
     *
     * @param data array of length pixels*region of interest
     * @param roi  region of interest. usually 300 or 1024 for FACT data
    ...
    /**
     * make an Array of the type int[] into ArrayList<Integer>. We need this because there is no
     * autoboxing between Integer and int types hence Arrays.aslist cannot be used for this
     * conversion
     */
    ...
    /**
     * This is a helper method which checks if all the keys provided are in the data item. If one of
     * the keys is not in the item a RuntimeException will be thrown containing a message detailing
     * which processor is causing the error
     */
    ...
    /**
     * This method tries to find the key in the data item and tries to cast them into the type given
     * by the type parameter. If it fails it will throw a RuntimeException with a message containing
    ...
    /**
     * Transforms camera coordinates (x, y) into longitudinal and transversal ellipse coordinates
     * (l, t). The ellipse coordinate system is defined by the center of gravity (x,y) and the angle
     * between the major axis and the camera x-axis (delta in radians).
     *
